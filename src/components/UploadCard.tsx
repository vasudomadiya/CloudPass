...
export default function UploadCard({ onUploadSuccess }: UploadCardProps) {
  ...
  const [codeType, setCodeType] = useState<"numeric" | "alphanumeric">(
    "numeric",
  );
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      const oversized = selectedList.find(
        (f) => f.size > 5 * 1024 * 1024 * 1024,
      );
      if (oversized) {
        setError("Each file must be less than 5GB.");
        return;
  ...
  return (
    <div
      id="upload"
      className="surface-glass border-glass rounded-[32px] p-6 md:p-8 flex flex-col gap-5 relative overflow-hidden h-full"
    >
      ...
      {!isUploading ? (
        <>
          ...
          {files.length === 0 ? (
            ...
            >
              ...
              <p className="text-sm text-slate-300 text-center px-4 transition-all duration-300">
                {isDragActive ? (
                  <span className="text-orange-400 font-bold text-base animate-pulse">
                    Release to drop your secure files! 🚀
                  </span>
                ) : (
                  <>
              ...
              <p
                className={`text-[11px] mt-2 transition-colors duration-300 ${isDragActive ? "text-orange-400/70 font-mono" : "text-slate-500"}`}
              >
                {isDragActive
                  ? "Processing multi-file buffer..."
                  : "Images, Videos, PDF, ZIP, DOCX, APK up to 5GB"}
              </p>
            </div>
          ) : (
            ...
            <div className="space-y-3">
              <div className="max-h-52 overflow-y-auto pr-1 space-y-2 scrollbar-thin scrollbar-thumb-white/10">
                {files.map((f, idx) => (
                  <div
                    key={idx}
                    className="bg-white/2 border border-white/5 rounded-xl p-3 flex items-center justify-between gap-3"
                  >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              <label
                htmlFor="expiry"
                className="text-xs font-mono font-medium text-slate-400 flex items-center gap-1.5 px-1"
              >
            ...
            <div className="space-y-1.5 sm:col-span-2">
              ...
              <div className="grid grid-cols-2 gap-3 mt-1">
                ...
                <button
                  type="button"
                  aria-pressed={codeType === "alphanumeric"}
        ...
        >
          ...
          <div className="text-center space-y-1.5">
            ...
            <h3 className="text-base font-extrabold text-white tracking-tight mt-3">
              {uploadPercent < 15
                ...
                : uploadPercent < 50
                  ? "Symmetric AES-256 Crypt..."
                  : uploadPercent < 90
                    ? "Streaming Encrypted Sectors..."
          ...
          <div className="space-y-2.5 bg-black/35 border border-white/5 p-4 rounded-2xl">
            ...
            <div
              className={`flex items-center gap-3 text-xs font-mono p-2 rounded-xl transition-all ${
                uploadPercent > 15
            ...
            >
              ...
              <div className="flex-1 text-left">
                ...
                <p className="text-[9px] text-slate-500 font-normal">
                  SHA-256 pre-flight checks complete
                </p>
              </div>
            </div>
            ...
            >
              <div className="flex-shrink-0">
                {uploadPercent > 50 ? (
                  ...
                  <div className="w-5 h-5 rounded-full border border-white/10 flex items-center justify-center text-[9px]">
                    2
                  </div>
                )}
              </div>
              <div className="flex-1 text-left">
                ...
                <p className="text-[9px] text-slate-500 font-normal">
                  {uploadPercent > 50
                    ? "Symmetric payloads sealed"
                    : uploadPercent > 15
                      ? "Encrypting sector chunks..."
            ...
            >
              ...
              <div className="flex-1 text-left">
                ...
                <p className="text-[9px] text-slate-500 font-normal">
                  {uploadPercent > 90
                    ? "Vault data blocks stored"
                    : uploadPercent > 50
                      ? `Streaming: ${uploadSpeed || "calculating..."}`
                      : "Awaiting payloads"}
                </p>
              </div>
            </div>
            ...
            >
              ...
              <div className="flex-1 text-left">
                <p className="font-semibold text-[11px]">
                  REPLICATING ACCESS INDEX
                </p>
                <p className="text-[9px] text-slate-500 font-normal">
                  {uploadPercent === 100
                    ? "Safe-Guard node index synced"
                    : uploadPercent > 90
                      ? "Verifying sector hashes..."
                      : "Awaiting lock registration"}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-between items-center text-[10px] font-mono text-slate-500">
              <span>
                ...
                {uploadPercent === 100
                  ? "Finalizing link..."
                  : uploadEta || "Awaiting speed..."}
              </span>
              <span>
                {formatBytes(totalSize * (uploadPercent / 100))} of{" "}
                {formatBytes(totalSize)}
              </span>
            </div>
          </div>
